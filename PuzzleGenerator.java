...
/**
 * PuzzleGenerator - Creates Sudoku puzzles of varying difficulty.
 * Generates random valid puzzles with unique solutions.
 */
class PuzzleGenerator {
     ...
     * @return Generated puzzle board (0 for empty cells)
     */
    int[][] generatePuzzle(int difficulty) {
        generateFullBoard();                     // Start with solved board
        int[][] puzzle = copyBoard(board);       // Copy to remove cells from
        
        int removals;  // Number of cells to clear based on difficulty
        if (difficulty == 1) removals = 25;      // Easy: 25 empty cells
        else if (difficulty == 2) removals = 45; // Medium: 45 empty cells
        else removals = 57;                      // Hard: 57 empty cells
        ...
        while (removals > 0) {
            ...
            int backup = puzzle[row][col];       // Save value
            puzzle[row][col] = 0;                // Try removing
            
            // Keep removal if puzzle still has unique solution
            if (solver.countSolutionsLimited(puzzle, difficulty)) {
                removals--;                      // Successful removal
            } else {
                puzzle[row][col] = backup;       // Restore value
            }
        }
        return puzzle;
    }
